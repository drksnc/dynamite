...
bool D3D11Hook::hook() {
    ...
    if (FAILED(hr)) {  
        spdlog::error(
            "Failed to create dummy D3D11 device. HRESULT={0:x} max_feature={1:x}", static_cast<uint32_t>(hr), static_cast<uint32_t>(device_max_feature_level));
        return false;
    }
    spdlog::info("Created dummy D3D11 device. HRESULT={0:x} max_feature={1:x}", static_cast<uint32_t>(hr), static_cast<uint32_t>(device_max_feature_level));
